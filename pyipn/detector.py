...
class Detector(object):
    ...
    def _check_earth_blockage(self, grb):
        ...
        grb_xyz = grb.location.get_cartesian_coord("gcrs").xyz/np.linalg.norm(grb.location.get_cartesian_coord("gcrs").xyz)
        

        

        horizon_angle = 0.5 * np.pi - np.arccos(earth_radius / self._location.altitude).to(u.rad).value

        sc_pos = self._location.get_cartesian_coord("gcrs").xyz/np.linalg.norm(self._location.get_cartesian_coord("gcrs").xyz)
    ...
    def build_light_curve(self, grb, T0, tstart, tstop, earth_blockage=False, seed=1234):
        """
